...
import { axisMax, paisaCompact, paisaFull, formatPct } from "@/lib/money";
      ...
      {holdings.length ? (
        <>
          {holdings.slice(0, 6).map((h) => (
            <div
              key={h.symbol}
              className="grid grid-cols-[1fr_auto_auto] items-center gap-4 border-t px-5 py-3 first:border-t-0 hover:bg-[var(--surface-1)]"
...
export function ExpensesPanel({
  ...
  return (
    <Panel title="Where it went" subtitle={`${paisaFull(expensesPaisa)} this month`}>
      {categories.length ? (
        <ExpenseBars items={categories.map((c) => ({ key: c.key, value: c.value / 100, pct: c.pct }))} />
      ) : (
        <p className="py-8 text-center text-[13px]" style={{ color: "var(--text-faint)" }}>
